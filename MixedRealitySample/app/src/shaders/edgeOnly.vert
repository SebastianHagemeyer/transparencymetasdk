...
layout(location = 0) out struct {
  ...
  vec3 worldNormal;
  vec3 worldPosition;
  vec3 objectPosition;  // object-space position for edge detection
} vertexOut;
...
layout (std140, set = 3, binding = 0) uniform MaterialUniform {
  ...
  vec4 edgeParams;       // x = thickness, y/z/w = reserved
} g_MaterialUniform;
...
void main() {
  ...
  vertexOut.objectPosition = app.position;  // pass object-space position
